    ...
    <div class="flex flex-col gap-6 w-full">
        ...
        <div class="relative overflow-x-auto p-5 flex justify-center">
            ...
            <div class="flex justify-center w-full max-w-screen-md shadow-md sm:rounded-lg p-3">
                <form class="w-full flex flex-col gap-5" action="{{ route('unidades.put', $unidad->id) }}" method="POST">
                    @csrf
                    <div class="flex flex-col w-full">
                        <label for="descripcion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            ...
                            <input type="text" id="descripcion" name="descripcion"
                                ...
                                placeholder="Kilogramo" value="{{ old('descripcion') ?? $unidad->descripcion }}" required />
                        </label>
                        @error('descripcion')
                        ...
                        <label for="descripcion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            ...
                            <input type="text" id="abreviatura" name="abreviatura"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500"
                                placeholder="kg" value="{{ old('abreviatura') ?? $unidad->abreviatura }}" required />
                        </label>
                        @error('abreviatura')
                            <p class="text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                            class="w-fit text-white bg-slate-700 hover:bg-slate-800 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-5 py-2 text-center dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">Actualizar</button>
                    </div>
